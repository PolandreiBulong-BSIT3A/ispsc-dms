...
const Favorite = ({ role, onOpenTrash, onNavigateToUpload, onNavigateToUpdate }) => {
  ...
  // Favorites source for this page
  const [favorites, setFavorites] = useState([]);
  // Document types lookup for mapping IDs -> names
  const [documentTypes, setDocumentTypes] = useState([]);
  // Loading flag for document types dropdown
  ...
  // Load users list (for resolving creator avatars) on mount
  useEffect(() => {
  ...
  const getCreatorName = (doc) => (
    doc?.created_by_name ||
    doc?.created_by ||
    doc?.creator_name ||
    doc?.owner_name ||
    doc?.created_by_full_name ||
    doc?.createdByName ||
    'Unknown'
  );

  // Resolve creator avatar URL from multiple possible fields and users list
  const getCreatorAvatar = (doc) => {
    // First check if this is the current user's document
    const isCurrentUserDoc = currentUser && (
      ...
      doc.created_by_id === currentUser.id ||
      doc.created_by_id === currentUser.user_id ||
      doc.creator_id === currentUser.id ||
      doc.creator_id === currentUser.user_id ||
      doc.owner_id === currentUser.id ||
      doc.owner_id === currentUser.user_id
    );

    // If it's the current user's document, use their profile picture
    if (isCurrentUserDoc && currentUser?.profilePic) {
      return currentUser.profilePic;
    }

    // Otherwise, try the existing logic
    const direct = doc?.created_by_profile_pic || doc?.created_by_avatar || doc?.profile_pic || doc?.profile_picture || doc?.created_by_photo || doc?.created_by_image;
    if (direct) return direct;
    
    // Try to resolve from usersList using id or name/email
    const docCreatorId = doc?.created_by_id || doc?.creator_id || doc?.owner_id || doc?.created_by_user_id || doc?.user_id;
    if (docCreatorId && Array.isArray(usersList) && usersList.length) {
      const u = usersList.find(x => String(x.user_id || x.id) === String(docCreatorId));
      if (u?.profile_pic || u?.avatar || u?.photo_url) return u.profile_pic || u.avatar || u.photo_url;
    }
    const name = getCreatorName(doc);
    if (name && Array.isArray(usersList) && usersList.length) {
      const lower = String(name).trim().toLowerCase();
      const u = usersList.find(x => [x.full_name, x.name, x.Username, x.username, x.email, x.user_email]
        .filter(Boolean)
  ...
  // Map doc_type numeric ID to readable name
  const mapDocType = (val, doc) => {
    // Prefer explicit name fields in the document itself
    const direct = doc?.doc_type_name || doc?.type_name || doc?.type_label;
    if (direct) return String(direct);
    if (val == null) return '-';
    const v = String(val).trim();
    // If already a non-numeric string, return as-is
    if (v && isNaN(Number(v))) return v;
    
    // Debug logging
    console.log('mapDocType debug:', {
      val,
      v,
    ...
    // Try map by id - check multiple possible ID fields
    const byId = documentTypes.find(t => {
      const typeId = String(t.type_id ?? t.id ?? t.value ?? t.doc_type_id ?? t.doc_type ?? '');
      return typeId === v;
    });
    
    if (byId) {
      const result = byId.name ?? byId.type_name ?? byId.label ?? byId.title ?? byId.text ?? v;
      console.log('Found mapping:', { byId, result });
      return result;
    }
    
    // Hardcoded fallback mapping for common document types
    ...
    if (hardcodedTypes[v]) {
      console.log('Using hardcoded mapping for doc_type:', v, '->', hardcodedTypes[v]);
      return hardcodedTypes[v];
    }
    
    // If no mapping found, return the original value or a fallback
    console.log('No mapping found for doc_type:', v, 'Available types:', documentTypes);
    return v || '-';
  };

  // Load favorites (current user's) on mount
  useEffect(() => {
    const load = async () => {
      try {
  ...
  useEffect(() => {
    const loadTypes = async () => {
      try {
        ...
        console.log('Document types loaded:', list);
  ...
  const handleToggleFavorite = async (docId) => {
    ...
    try {
      ...
      // Refresh favorites list for this page
      try {
        const data = await fetchFavoriteDocuments();
        setFavorites(Array.isArray(data?.favorites) ? data.favorites : (Array.isArray(data) ? data : []));
      } catch {}
  ...
  const filteredDocuments = useMemo(() => {
    ...
    console.log('Total documents to filter (favorites):', favorites.length);
    let filtered = favorites.filter(doc => {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        // Normalize to strings for localeCompare
    ...
    return filtered;
  }, [favorites, lowerSearch, selectedCategory, selectedDepartment, showOnlyMyDocuments, currentUser, effectiveIsDean, isAdmin, isUser, sortConfig, userPreferences]);

  // Build list of folders that actually contain at least one currently visible document
  const visibleFolders = useMemo(() => {
    const counts = new Map();
    filteredDocuments.forEach(doc => {
      // tally folder names for visible docs
  ...
  // Fetch departments on component mount (document types are fetched above)
  useEffect(() => {
    fetchDepartments();
  }, [fetchDepartments]);
  ...
  const categories = useMemo(() => {
    ...
    if (documentTypes.length > 0) {
      // Convert to string names if items are objects
      const names = documentTypes.map(dt => (dt?.name ?? dt?.type_name ?? dt?.label ?? dt?.title ?? dt?.text ?? dt));
      return names.filter(Boolean).sort();
    }
    
    // Fallback: extract unique doc types from documents
    const uniqueTypes = [...new Set(documentsToUse.map(doc => doc.doc_type).filter(Boolean))];
    return uniqueTypes.sort();
  }, [documentTypes, documents, effectiveIsDean, isAdmin, isUser, currentUser, showOnlyMyDocuments]);
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Header */}
        ...
        {/* Removed header actions for Favorites */}
        </div>
          {/* Search and Filters */}
      <div style={{
        ...styles.controls,
        ...
        <div style={{
          ...
          {/* Filters button removed for Favorites view */}
          ...
          <tbody>
                 ...
                 <><tr
                         ...
                         >
                           ...
                           <td style={styles.td} className="hide-on-small">{mapDocType(doc.doc_type, doc) || '-'}</td>
                           <td style={{ ...styles.td, ...styles.tableRowLastCell }} className="hide-on-medium">
                             <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                               {(() => {
                                 const avatarUrl = getCreatorAvatar(doc); return avatarUrl ? (
                               ...
                               <div>
                                 <div style={{ fontWeight: 500, fontSize: 14 }}>{getCreatorName(doc)}</div>
                  ...
                  {hasAdminPrivileges(role) && (
                    <>
                        ...
                        <div style={styles.gridCardMeta}>
                          {getCreatorAvatar(doc) ? (
                            <img src={getCreatorAvatar(doc)} alt="avatar" style={styles.gridCardAvatar} />
                          ) : (
                            <div style={{...styles.gridCardAvatar, background: getColorFromString(
                              doc.created_by_name ||
                              doc.created_by ||
                              doc.creator_name ||
                              doc.owner_name ||
          ...
          {paginatedDocuments.map((doc) => (
            <div key={doc.id} style={styles.listItem}>
              <div style={styles.listItemLeft}>
                ...
                {getCreatorAvatar(doc) ? (
                  <img src={getCreatorAvatar(doc)} alt="avatar" style={styles.listItemAvatar} />
                ) : (
                  <div
                    style={{
                ...
                <div style={styles.listItemInfo}>
                  ...
                  <div style={styles.listItemMeta}>
                    {getCreatorName(doc)} • {doc.created_at ? new Date(doc.created_at).toLocaleDateString() : 'N/A'}
