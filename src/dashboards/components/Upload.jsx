...
const categories = [];
// Frontend-only fallback actions when API is not available
const defaultActionOptions = [
  { id: 1, name: 'For Review' },
  { id: 2, name: 'For Approval' },
  { id: 3, name: 'For Signature' },
  { id: 4, name: 'For Notation' },
  { id: 5, name: 'For Information' },
];

const Upload = ({ role, onNavigateToDocuments }) => {
  // Debug: Log the role value
  console.log('Upload component - Role received:', role);
  console.log('Role type:', typeof role);
  console.log('Role is dean:', role?.toLowerCase() === 'dean');
  console.log('Role is admin:', role?.toLowerCase() === 'admin');
  console.log('Role is faculty:', role?.toLowerCase() === 'faculty');
  console.log('Should show visibility section:', (role?.toLowerCase() === 'dean' || role?.toLowerCase() === 'admin' || role?.toLowerCase() === 'faculty'));
  ...
  const [selectedPreviewLink, setSelectedPreviewLink] = useState(0); // Index of selected link for preview
  ...
  // Prefill form from sessionStorage (for reply functionality)
  useEffect(() => {
    try {
      const prefillData = sessionStorage.getItem('uploadPrefill');
      if (prefillData) {
        const data = JSON.parse(prefillData);
        console.log('Prefilling upload form with data:', data);
        
        // Prefill basic fields
        if (data.title) setLinkTitle(data.title);
        if (data.reference) setDocReference(data.reference);
        ...
        if (data.category) setDocType(data.category);
        if (data.available_copy) setAvailableCopy(data.available_copy);
        
        // Prefill Google Drive link
        if (data.google_drive_link) {
          setMultipleLinks([{ id: 1, link: data.google_drive_link }]);
        }
        
        // Set flag to indicate this is a reply/prefilled document
        setIsPrefilledFromAnswered(true);
        
        // Clear the sessionStorage after prefilling
        sessionStorage.removeItem('uploadPrefill');
      }
    } catch (e) {
      console.warn('Failed to prefill upload form:', e);
    }
  }, []);

  // Prefill form from sessionStorage (for createReply functionality from Request component)
  useEffect(() => {
    try {
      const createReplyData = sessionStorage.getItem('createReply');
      if (createReplyData) {
        const data = JSON.parse(createReplyData);
        console.log('Prefilling upload form with createReply data:', data);
        ...
        // Handle action required fields if present
        if (data.action_required && Array.isArray(data.action_required) && data.action_required.length > 0) {
          setRequireAction(true);
          // Map action names to IDs if possible
          const actionIds = data.action_required.map(actionName => {
            const found = actionOptions.find(opt => opt.name === actionName);
        ...
        // Show success message indicating the type of prefill
        if (data.isRequest) {
          setSuccessMessage('Form pre-filled from request! You can modify the details as needed.');
        } else if (data.isNewDocument) {
          setSuccessMessage('Form pre-filled for new document! You can modify the details as needed.');
        } else {
          setSuccessMessage('Form pre-filled from document! You can modify the details as needed.');
        }
      ...
      console.warn('Failed to prefill upload form from createReply:', e);
    }
  }, [actionOptions]); // Depend on actionOptions so we can map action names to IDs
  ...
  const addDriveLink = async (e) => {
    ...
    try {
      // Process each valid link as a separate document
      const validLinks = multipleLinks.filter(link => link.link.trim());
      ...
      const newDriveLink = {
        id: Date.now() + Math.random(),
          title: uniqueTitle, // Auto-generate unique title with suffix
          link: linkData.link,
      ...
      setDriveLinks(prev => [...prev, newDriveLink]);
      ...
      const documentForContext = {
        ...
        category: docType,
        folder: selectedFolder, // Add folder field
        ...(revision && { revision }),
  ...
  const removeDriveLink = (linkId) => {
    setDriveLinks(prev => prev.filter(link => link.id !== linkId));
  };
      ...
      <div style={{...styles.flexCard, maxWidth: isMobile ? '100%' : 1200}}>
        <div style={{...styles.formCol, padding: isMobile ? '16px' : '32px'}}>
          <div style={{ 
            ...
            <div>
              <h2 style={{...styles.title, fontSize: isMobile ? '24px' : '28px'}}>Upload Google Drive Link</h2>
              {isPrefilledFromAnswered && (
                <div style={{
                  ...
                  <FiCheck size={14} />
                  Pre-filled from reply request - modify as needed
                </div>
              )}
            </div>
            <div style={{ 
